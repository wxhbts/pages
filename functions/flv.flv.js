...
async function handleRequest(request) {
  ...
  if (!videoUrl) {
    return new Response('缺少 URL 参数。 请使用 flv.flv?url=您的视频URL', { status: 400 });
  }
  ...
  try {
    ...
    if (!configResponse.ok) {
      return new Response(`获取配置失败: ${configResponse.status} ${configResponse.statusText}`, { status: configResponse.status });
    }
    ...
    if (!videoResponse.ok) {
      return new Response(`获取视频失败: ${videoResponse.status} ${videoResponse.statusText}，URL: ${videoUrl}`, { status: videoResponse.status });
    }
    ...
    return new Response(`获取视频时发生错误: ${error.message}`, { status: 500 }); // 改为 500 表示服务器内部错误
  }
}
